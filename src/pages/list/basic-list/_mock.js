...
const missionNums = [];
for (let i = 0; i < 5; i++) {
    missionNums.push(Math.floor(Math.random() * 100))
}
// const titles = [
//     '新任务',
//     '预约任务',
...
const createTag = () => {
    let _list = []
    for (let i = 0; i < 3; i++) {
        _list.push(tagList[Math.floor(Math.random() * tagList.length)])
    }
...
function fakeList(count) {
    ...
    for (let i = 0; i < count; i += 1) {
        list.push({
            ...
            content: '121212段落示意：蚂蚁金服设计平台 ant.design，用最小的工作量，无缝接入蚂蚁金服生态，提供跨越设计与开发的体验解决方案。蚂蚁金服设计平台 ant.design' +
            ...
            // 任务列表数据
            type: 'sddsd',
            // CName: cNameList[Math.floor(Math.random() * cNameList.length)],
            // tag: createTag(),
            // batchNum: moment().format('YYYYMMDDSSS'),
            // dateTime: moment().format('YYYY/MM/DD'),
            // content: '这是一个客户的备注,可能会很长',
            // lastCallState: Math.floor(Math.random() * 2)
        });
    }
    return list;
}
